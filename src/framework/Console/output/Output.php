...
use framework\Console\output\Writer\WriterInterface;
use framework\Console\output\Writer;
use framework\Console\output\formatter\Style;
use framework\Console\Helper;
...
class Output
{
    ...
    {
        $this->add('output', new Writer\StreamWrite);
        $this->add('error', new Writer\StreamWriteErr);
        $this->add('buffer', new Writer\Buffer);

        $this->defaultTo('output');
    ...
    {
		if($this->style){
			$content = $this->style->text($content);
		}
    ...
    protected function handleUnknownWriter($writer)
    {
        ...
        if (is_object($writer)) {
            throw new InvalidArgumentException('Class [' . get_class($writer) . '] must implement '
                                    . 'framework\Console\output\Writer\WriterInterface.');
        }
