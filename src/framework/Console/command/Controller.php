...
class Controller extends Command
{
    public function configure()
    {
		...
		$this->addArgument('name',function($argument){
			$argument->setDescription('The name of the controller');
    ...
    public function execute(Input $input, Output $output)
    {
        ...
        $content = str_replace(['{%controller_name%}', '{%path_name%}'], [
            $name,
			$path
        ], $content);
	...
	public function getPath(){
		return  implode(DIRECTORY_SEPARATOR,[__DIR__,'templates','controller.phpt']);
	}
	
	public function getStorePath($path,$name){
		return implode(DIRECTORY_SEPARATOR,[$path,'Controllers',"{$name}.php"]);
	}
}
